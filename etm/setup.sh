...
NGINX_SITE=/etc/nginx/sites-available/etm
NGINX_ENABLED_SITE=/etc/nginx/sites-enabled/etm

DB_NAME=${DB_NAME:-etm}
DB_USER=${DB_USER:-etm}

echo "=== ETM bootstrap v${SCRIPT_VERSION} ==="
...
load_or_create_secrets() {
  ...
  ensure_secret ETM_SECURITY_USERNAME "Enter ETM admin username [etm]: " false etm
  ensure_secret ETM_SECURITY_PASSWORD "Enter ETM admin password: " true
...
server {
    ...
    location = /apk {
        ...
        add_header Content-Disposition 'attachment; filename="etm.apk"';
    }
...
echo "===================================="
echo " ETM server is ready for deploy"
echo " Script version: ${SCRIPT_VERSION}"
echo " Public domain: https://${APP_DOMAIN}"
echo " App port: ${APP_PORT}"
echo " Nginx site: ${NGINX_SITE}"
echo " Next step: run etm/deploy.sh"
