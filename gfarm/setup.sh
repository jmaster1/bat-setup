...
APP_USER=gfarm
APP_DIR=/opt/gfarm
APP_DOMAIN=gfarm.jmaster.online
WWW_DIR=${APP_DIR}/www
SECRETS_FILE=${APP_DIR}/secrets.txt
PRIVACY_POLICY_URL=${PRIVACY_POLICY_URL:-https://raw.githubusercontent.com/jmaster1/bat-setup/main/gfarm/privacy_policy.html}

NGINX_SITE=/etc/nginx/sites-available/gfarm
NGINX_ENABLED_SITE=/etc/nginx/sites-enabled/gfarm

echo "=== GFarm static site bootstrap v${SCRIPT_VERSION} ==="
...
install_privacy_policy() {
  ...
  if ! grep -qi '<h2>Privacy Policy</h2>' "$tmp_file"; then
    echo "Downloaded file does not look like the GFarm privacy policy: ${PRIVACY_POLICY_URL}" >&2
    rm -f "$tmp_file"
    exit 1
  fi
...
echo "===================================="
echo " GFarm static site is ready"
